...
export function currentYearSeason() {
	let today = new Date()
...
export function toAMPM(time) {
	if(time.timeInHours) return toAMPM(Math.floor(time.timeInHours)+':'+Math.round(60*(time.timeInHours - Math.floor(time.timeInHours))).toString().padStart(2, '0'))
...
export function dayofweekFromDayandTime(day, time) { 
	...
	if(day == 6 && dayChange == 1) return 0
	if(day == 0 && dayChange == -1) return 6
...
export function pqueueAll(promisesArray) {
